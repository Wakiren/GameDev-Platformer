...
// Blit to screen
bool Render::DrawTexture(SDL_Texture* texture, int x, int y, const SDL_Rect* section, float speed, double angle, int pivotX, int pivotY, float scaleFactor, Uint8 alpha) const
{
	bool ret = true;
	int scale = Engine::GetInstance().window.get()->GetScale();
	...
	if(section != NULL)
	{
		rect.w = section->w * scaleFactor;
		rect.h = section->h * scaleFactor;
	}
	else
	{
